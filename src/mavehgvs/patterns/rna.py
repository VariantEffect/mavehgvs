...
rna_equal: str = (
    rf"(?P<rna_equal>(?:(?:(?P<start>{pos_intron})_"
    + rf"(?P<end>{pos_intron}))|(?P<position>{pos_intron}))?(?P<equal>=))"
)
...
"""str: Pattern matching a RNA substitution with numeric or relative-to-transcript
positions.
"""

rna_del: str = (
    rf"(?P<rna_del>(?:(?:(?P<start>{pos_intron})_(?P<end>{pos_intron}))|"
    + rf"(?P<position>{pos_intron}))del)"
)
"""str: Pattern matching a RNA deletion with numeric or relative-to-transcript
positions.
"""

rna_dup: str = (
    rf"(?P<rna_dup>(?:(?:(?P<start>{pos_intron})_(?P<end>{pos_intron}))|"
    + rf"(?P<position>{pos_intron}))dup)"
)
"""str: Pattern matching a RNA duplication with numeric or relative-to-transcript
positions.
"""
...
"""str: Pattern matching a RNA insertion with numeric or relative-to-transcript
positions.
"""

rna_delins: str = (
    rf"(?P<rna_delins>(?:(?:(?P<start>{pos_intron})_(?P<end>{pos_intron}))|"
    + rf"(?P<position>{pos_intron}))delins(?P<seq>{rna_nt}+))"
)
"""str: Pattern matching a RNA deletion-insertion with numeric or relative-to-transcript
positions.
"""
...
rna_multi_variant: str = (
    rf"(?P<rna_multi>r\.\[{remove_named_groups(rna_variant)}"
    + rf"(?:;{remove_named_groups(rna_variant)}){{1,}}\])"
)
